...
function indekseGoreFenomen(/*kod*/fenomenler, indeksSayisi) {
  /*kod*/
/*
  for (let i = 0; i < fenomenler.length; i ++) {
    
    if( i === indeksSayisi){
      let result = `${indeksSayisi}. indekste bulunan fenomen: ${fenomenler[indeksSayisi].profile}`;
      return result;
    }
  ...
  */
  ...
  const secilenFeno = fenomenler[indeksSayisi];
  const {profile} = secilenFeno;
  return `${indeksSayisi}. indekste bulunan fenomen: ${profile}`
...
function profilListesi(/*kod*/kaynakDizi ) {

  /*
  let kopyaDizi = [];
  kaynakDizi = [...kopyaDizi];
  let yeniDizi;
  yeniDizi = kopyaDizi;
  return yeniDizi;
  ...
  */
  ...
  const yeniDizi = [...kaynakDizi];

  for (let i = 0 ; i <kaynakDizi.length; i++){
    yeniDizi[i]=kaynakDizi[i].profile;
  }

 /*
 const yeniDizi = [...kaynakDizi];
 for (const item of kaynakDizi) {
...
function fenomenSil(/*kod*/birDizi, birIndex) {
  const yeniDizi = [...birDizi];
  yeniDizi.splice(birIndex, 1,);
  return yeniDizi;
...
function fenomenEkle(/*kod*/birDizi, number, profile, followers, posts, platform) {
  /*kod*/
  const yeniDizi = [...birDizi];
  let newObj= {
    number,
...
function enFenomenler(/*kod*/birDizi) {
  /*kod*/
  let yeniDizi = [];
  for(let i=0; i<birDizi.length; i++){
    if(birDizi[i].followers > 100000000){
      yeniDizi.push(birDizi[i].profile);
    }
...
function fenomenGonderimSayisi(/*kod*/birDizi, profile){
  /*kod*/
  let result = 0;
  for(let i=0; i<birDizi.length; i++){
    if(birDizi[i].profile === profile){
      result = birDizi[i].posts;
    }
...
function platformaGoreCokGonderiYapanFenomen(birDizi, birPlatform){
  ...
  for(let i=0; i<birDizi.length; i++){
    if(birDizi[i].platform === birPlatform){
      newDizi.push(birDizi[i]); 
      }

    }
    let enBuyukSayi= newDizi[0];
  for(let i=0; i<newDizi.length; i++){
    if(newDizi[i].posts > enBuyukSayi.posts ) {
      enBuyukSayi = newDizi[i];
    }
  } 
  return enBuyukSayi.profile;
  }
